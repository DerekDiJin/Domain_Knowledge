...
for i = 1:1:3
    ...
    for j = 1:1:2
        ...
        s_2 = (sum(sum(SF(Random_F, Random_F))) - length(scagnostics_F))/2;
        h_2 = sum(h(Random_F));

        % score 4
        [xLinInt fval iter] = MIQP_fixed (SF, h, 0, 0, lambda3, length(scagnostics_F))
        Surprising_F = find(xLinInt==1)';
        s_4 = (sum(sum(SF(Surprising_F, Surprising_F))) - length(scagnostics_F))/2;
        h_4 = sum(h(Surprising_F));
        
        s_set_t(end+1, :) = [s_1 s_2 s_3 s_4];
        h_set_t(end+1, :) = [h_1 h_2 h_3 h_4];
    end
...
legend_str = {'EAGLE:S_F', 'EAGLE:h', 'Random:S_F', 'Random:h', 'Scagnostics:S_F', 'Scagnostics:h', 'Surprising:S_F', 'Surprising:h'};
