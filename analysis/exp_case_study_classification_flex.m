...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Baseline 2: re-organize the adj matrices
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dataFolder_train = '../DATA/real_pre/neural_pearson0.85/';
...
filePattern_train = fullfile(dataFolder_train, '*.txt');
...
all = zeros(148, 1166*1166);

for i=1:N_train
    
    curFile_name = strcat(dataFolder_train, theFiles_train_names_str(i,:))
    data = load(curFile_name);
    data_matrix = sparse(data(:,1), data(:,2), 1, 1166, 1166);
    data_matrix = full(sparse(data(:,1), data(:,2), 1, 1166, 1166));
    all(i,:) = reshape(data_matrix', [1, 1166*1166]);
end

baseline2_Xs_c = all(indices_input_c,:);
baseline2_Xs_p = all(indices_input_p,:);
baseline2_Xs = [baseline2_Xs_c; baseline2_Xs_p];
baseline2_Ys = baseline1_Ys;
